...
# Removed global connections to support multithreading
# Connections are now created within each handler
...
def requestSaltPathHandler(self):
    ...
    with sqlite3.connect(AUTH_DB) as authDBConnection:
...
def getDogRaceInfoPathHandler(self):
    ...
    with sqlite3.connect(RACE_DB) as raceDBConnection:
        ...
        if result:
            ...
            if photo_bytes is not None:
                ...
                self.send_header('Connection', 'close')
                self.end_headers()
                self.wfile.write(json.dumps(response_metadata).encode('utf-8'))
